...
Tem **executável pronto para Windows e Linux**, sem Python e sem instalação —
veja [Executável pronto](#executável-pronto-windows-e-linux).

<!-- Capturas de tela: coloque as imagens em docs/ e descomente.
![Aba Tripulação](docs/screenshot-crew.png)
![Aba Armazenamento](docs/screenshot-storage.png)
-->

## Por que mais um editor de save

Existem outros editores, e são bons. Este existe pelo que eles não cobrem:

- **Linux é alvo de primeira classe** — binário nativo, não gambiarra.
- **Português completo**, inclusive os nomes que o próprio jogo usa para
  recursos, tecnologias, traços e perícias. Eles vêm da sua cópia do jogo, não
  de uma tradução feita à mão.
- **Gravação byte a byte idêntica.** Carregar e gravar sem mexer em nada
  devolve arquivos iguais aos originais; editar um atributo muda exatamente
  aquela linha. Nada mais no arquivo é reescrito, reordenado ou reformatado.
- **Uma aba de XML cru** com busca em todo o save, para o que as abas prontas
  não expõem.
- **Sem dependências.** Só a biblioteca padrão do Python, então dá para ler o
  código inteiro e rodar do fonte em vez de confiar num binário.
...
tools/compare_galaxy.py      compara a galáxia gerada de dois ou mais saves
...
`shedit/data/gamedata.json` pertencem à Bugbyte Ltd. e não estão cobertos por
ela — veja o [NOTICE](NOTICE) e o [aviso legal](#aviso-legal).
